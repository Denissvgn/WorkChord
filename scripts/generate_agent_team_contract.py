...
DEFAULT_OUTPUT = REPOSITORY_ROOT / "docs" / "contracts" / "agent-team-master-v1.schema.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate the agent-team-master-v1 JSON Schema.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def rendered_schema() -> bytes:
    ...
    schema = AgentTeamMaster.model_json_schema(
        ref_template="#/$defs/{model}",
        mode="validation",
    )
    schema["$id"] = "https://workchord.local/contracts/agent-team-master-v1.schema.json"
